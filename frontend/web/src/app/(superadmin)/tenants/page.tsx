...
export default function TenantsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: CreateTenantRequest) => apiClient.post("/tenants", data).then((r) => r.data),
    onSuccess: () => {
  ...
  if (isLoading) {
    return <div className="text-muted-foreground">Cargando tenants...</div>;
  }
        ...
        <table className="w-full text-sm">
          <thead className="border-b border-border bg-muted/50">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-medium">Ambiente SRI</th>
              <th className="px-4 py-3 text-left font-medium">Comp. mes</th>
          ...
          <tbody>
            {tenants.map((tenant) => (
              <tr key={tenant.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                <td className="px-4 py-3 font-mono">{tenant.ruc}</td>
                <td className="px-4 py-3">{tenant.razon_social}</td>
                <td className="px-4 py-3">
                  <TenantEstadoBadge estado={tenant.estado} />
                </td>
                <td className="px-4 py-3">
                  <span className={`text-xs font-medium ${tenant.ambiente_sri === "PRODUCCION" ? "text-green-600" : "text-yellow-600"}`}>
                    {tenant.ambiente_sri}
                  </span>
                </td>
  ...
  return (
    <div className="rounded-lg border border-border bg-card p-6 space-y-4">
      ...
      <div className="flex gap-3 justify-end">
        <button onClick={onCancel} className="rounded-md px-4 py-2 text-sm border border-input hover:bg-muted">
          Cancelar
        </button>
        <button
          onClick={() => onSubmit(form)}
          disabled={isLoading || !form.ruc || !form.razon_social}
