...
const ESTADOS_EN_PROGRESO = ["QUEUED", "PROCESSING", "SENT_TO_SRI", "PENDING_AUTHORIZATION", "EMAIL_PENDING"];
...
export function ComprobanteDetallePage() {
  ...
  const { data: comp, isLoading } = useQuery<Comprobante>({
    ...
    refetchInterval: (query) => {
      ...
      return ESTADOS_EN_PROGRESO.includes(estado) ? 3000 : false;
    },
    enabled: !!id,
  });
  ...
  const retryMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["comprobante", id] }),
  });

  const handleDescargar = async (tipo: "xml" | "pdf") => {
    const { data } = await apiClient.get<{ url: string }>(`/comprobantes/${id}/descargar/${tipo}`);
    ...
    <div className="max-w-2xl space-y-6">
              ...
              <div key={e} className="flex items-center flex-1">
                <div className="flex flex-col items-center gap-1">
                  <div className={`w-3 h-3 rounded-full border-2 ${
                    ...
                    done ? "border-green-500 bg-green-500" : "border-border bg-background"
                  }`} />
      ...
      <div className="flex flex-wrap gap-2">
        {comp.numero_autorizacion && (
          <>
            <button onClick={() => handleDescargar("xml")} className="rounded-md border border-input px-3 py-1.5 text-sm hover:bg-muted">Descargar XML</button>
            <button onClick={() => handleDescargar("pdf")} className="rounded-md border border-input px-3 py-1.5 text-sm hover:bg-muted">Descargar PDF</button>
          </>
        )}
        {["RETURNED_BY_SRI", "FAILED"].includes(comp.estado) && (
          <button onClick={() => retryMutation.mutate("REENVIAR_SRI")} disabled={retryMutation.isPending} className="rounded-md bg-primary px-3 py-1.5 text-sm text-primary-foreground hover:opacity-90 disabled:opacity-50">Reenviar al SRI</button>
        )}
        {["NOT_AUTHORIZED", "MANUAL_REVIEW_REQUIRED"].includes(comp.estado) && (
          <button onClick={() => retryMutation.mutate("RECONSULTAR_AUTORIZACION")} disabled={retryMutation.isPending} className="rounded-md bg-primary px-3 py-1.5 text-sm text-primary-foreground hover:opacity-90 disabled:opacity-50">Reconsultar</button>
        )}
        {["EMAIL_FAILED", "AUTHORIZED"].includes(comp.estado) && (
          <button onClick={() => retryMutation.mutate("REENVIAR_EMAIL")} disabled={retryMutation.isPending} className="rounded-md border border-input px-3 py-1.5 text-sm hover:bg-muted">Reenviar email</button>
        )}
      </div>
    </div>
  );
