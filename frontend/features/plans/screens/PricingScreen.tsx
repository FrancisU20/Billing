  ...
  return (
    <View style={[staticStyles.container, { backgroundColor: semantic.bg.page }]}>
      <View style={[staticStyles.hero, { backgroundColor: semantic.bg.elevated, borderColor: semantic.border.default }]}>
        <View style={[staticStyles.heroBadge, { backgroundColor: semantic.accent.altSubtle, borderColor: semantic.border.default }]}>
        ...
        <View style={staticStyles.heroCopy}>
          <Text style={[staticStyles.title, { color: semantic.text.primary }]}>Facturación electrónica SRI</Text>
          <Text style={[staticStyles.subtitle, { color: semantic.text.secondary }]}>
            Elige una base clara para emitir documentos, crecer por usuarios y activar módulos cuando tu operación lo requiera.
          </Text>
        </View>

        <View style={staticStyles.signalRow}>
          {pricingSignals.map((signal) => (
            <View key={signal.label} style={[staticStyles.signalPill, { backgroundColor: semantic.bg.muted, borderColor: semantic.border.default }]}>
              <Ionicons name={signal.icon} size={14} color={semantic.accent.tertiary} />
              <Text style={[staticStyles.signalText, { color: semantic.text.secondary }]}>{signal.label}</Text>
            </View>
          ))}
        </View>
      </View>
      ...
      {error ? (
        ...
        <EmptyState icon="pricetags-outline" title="Sin planes disponibles"
        ...
        <FlatList
          data={activePlans}
          keyExtractor={(p) => p.id}
          renderItem={({ item, index }) => <PlanCard plan={item} highlighted={index === highlightedIndex} />}
...
const staticStyles = StyleSheet.create({
  ...
  title: { fontSize: typography.size['3xl'], fontWeight: typography.weight.bold, lineHeight: typography.size['3xl'] * typography.lineHeight.tight },
  subtitle: { fontSize: typography.size.base, lineHeight: typography.size.base * typography.lineHeight.normal },
  signalRow: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing[2] },
  signalPill: { flexDirection: 'row', alignItems: 'center', gap: spacing[1], borderWidth: 1, borderRadius: radius.full, paddingHorizontal: spacing[3], paddingVertical: spacing[2] - 1 },
