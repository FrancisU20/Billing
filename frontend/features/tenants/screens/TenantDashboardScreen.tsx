...
const modulePreview = [
  ...
  { icon: 'return-down-back-outline', title: 'Notas de crédito', subtitle: 'Ajustes y anulaciones' },
      ...
      <ScrollView contentContainerStyle={staticStyles.scroll} showsVerticalScrollIndicator={false}>
        <View style={[staticStyles.hero, { backgroundColor: semantic.bg.elevated, borderColor: semantic.border.default }]}>
          ...
          <View style={staticStyles.heroTop}>
            ...
            <View style={[staticStyles.rolePill, { backgroundColor: semantic.accent.subtle, borderColor: semantic.border.default }]}>
              <Text style={[staticStyles.roleText, { color: semantic.accent.default }]} numberOfLines={1}>{roleLabel}</Text>
            </View>
          </View>

          <View style={staticStyles.heroCopy}>
            <Text style={[staticStyles.heroTitle, { color: semantic.text.primary }]}>Centro de facturación</Text>
        ...
        <Card variant="elevated" elevated style={staticStyles.insightCard}>
          <View style={staticStyles.insightHeader}>
            <View style={[staticStyles.insightIcon, { backgroundColor: semantic.accent.tertiarySubtle }]}>
              <Ionicons name="analytics-outline" size={20} color={semantic.accent.tertiary} />
            </View>
            <View style={staticStyles.insightCopy}>
              <Text style={[staticStyles.insightTitle, { color: semantic.text.primary }]}>Flujo del mes</Text>
          ...
          <View style={[staticStyles.progressTrack, { backgroundColor: semantic.chart.track }]}>
            <View style={[staticStyles.progressFill, { backgroundColor: semantic.chart.tertiary }]} />
          </View>
        </Card>
        ...
        <Card variant="elevated" elevated style={staticStyles.modulesCard}>
          {modulePreview.map((module) => (
            <View key={module.title} style={[staticStyles.moduleRow, { borderBottomColor: semantic.border.default }]}>
              <View style={[staticStyles.moduleIcon, { backgroundColor: semantic.accent.altSubtle }]}>
                <Ionicons name={module.icon} size={18} color={semantic.accent.alt} />
              </View>
              <View style={staticStyles.moduleCopy}>
                <Text style={[staticStyles.moduleTitle, { color: semantic.text.primary }]}>{module.title}</Text>
                <Text style={[staticStyles.moduleSubtitle, { color: semantic.text.secondary }]}>{module.subtitle}</Text>
              </View>
...
function HeroSignal({ label, value }: { label: string; value: string }) {
  ...
  return (
    <View style={staticStyles.heroSignal}>
      <Text style={[staticStyles.heroSignalLabel, { color: semantic.text.secondary }]}>{label}</Text>
...
function SectionHeader({ title, caption }: { title: string; caption: string }) {
  ...
  return (
    <View style={staticStyles.sectionHeader}>
      ...
      <Text style={[staticStyles.sectionCaption, { color: semantic.text.secondary }]}>{caption}</Text>
    </View>
  )
}
  ...
  return (
    <Card variant="elevated" elevated style={staticStyles.metricCard}>
      ...
      <View style={staticStyles.metricCopy}>
        ...
        <Text style={[staticStyles.metricLabel, { color: semantic.text.primary }]} numberOfLines={2}>{label}</Text>
        <Text style={[staticStyles.metricDetail, { color: semantic.text.secondary }]}>{detail}</Text>
      </View>
    </Card>
  )
}
...
const staticStyles = StyleSheet.create({
  ...
  kicker: { fontSize: typography.size.xs, fontWeight: typography.weight.semibold, textTransform: 'uppercase' },
  email: { fontSize: typography.size.base, fontWeight: typography.weight.semibold },
  rolePill: { maxWidth: 118, borderWidth: 1, borderRadius: radius.full, paddingHorizontal: spacing[3], paddingVertical: spacing[1] },
  roleText: { fontSize: typography.size.xs, fontWeight: typography.weight.semibold },
  heroCopy: { gap: spacing[2] },
  heroTitle: { fontSize: typography.size['3xl'], fontWeight: typography.weight.bold, lineHeight: typography.size['3xl'] * typography.lineHeight.tight },
  heroSubtitle: { fontSize: typography.size.base, lineHeight: typography.size.base * typography.lineHeight.normal },
  ...
  metricIcon: { width: 44, height: 44, borderRadius: radius.lg, alignItems: 'center', justifyContent: 'center' },
  metricCopy: { flex: 1, gap: spacing[1] },
  metricValue: { fontSize: typography.size['2xl'], fontWeight: typography.weight.bold, lineHeight: typography.size['2xl'] * typography.lineHeight.tight },
  ...
  insightIcon: { width: 44, height: 44, borderRadius: radius.lg, alignItems: 'center', justifyContent: 'center' },
  insightCopy: { flex: 1, minWidth: 0, gap: spacing[1] },
  insightTitle: { fontSize: typography.size.md, fontWeight: typography.weight.bold },
  insightDescription: { fontSize: typography.size.sm, lineHeight: typography.size.sm * typography.lineHeight.normal },
  ...
  moduleRow: { flexDirection: 'row', alignItems: 'center', gap: spacing[3], paddingVertical: spacing[3], borderBottomWidth: 1 },
  moduleIcon: { width: 40, height: 40, borderRadius: radius.lg, alignItems: 'center', justifyContent: 'center' },
  ...
  moduleFooterText: { fontSize: typography.size.sm, lineHeight: typography.size.sm * typography.lineHeight.normal },
})
