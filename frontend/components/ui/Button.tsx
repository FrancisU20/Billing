...
export function Button({ children, variant = 'primary', size = 'md', isLoading = false, isDisabled = false, fullWidth = false, style: externalStyle, ...props }: ButtonProps) {
  ...
  return (
    ...
    >
      {isLoading ? (
        <ActivityIndicator
          size="small"
          color={variant === 'primary' || variant === 'danger' ? overlay.text.primary : semantic.accent.default}
          style={{ marginRight: spacing[2] }}
        />
      ) : null}
      <Text style={[staticStyles.label, staticStyles[`${size}Label` as keyof typeof staticStyles], { color: variantText[variant] }]}>
        {children}
      </Text>
    </Pressable>
  )
