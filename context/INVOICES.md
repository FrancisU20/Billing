...
- 2026-06-29: `features/documents/utils.ts` centraliza `toNumber()`/`round2()` para los
  previews de factura y nota de credito. `round2()` ahora usa round-half-even para
  alinearse con `Decimal.quantize()` del backend; `creditNoteForm.test.ts` y
  `utils.test.ts` cubren escalado de NC y empates de redondeo.
