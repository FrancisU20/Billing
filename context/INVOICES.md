...
> Renombrada de "Deuda Tecnica Anticipada" — el motor SRI ya esta en produccion real
> (Sprints 1-6 + Nota de Credito desplegados), "anticipada" ya no describe el estado actual.
...
| RIDE omite la fila de IVA 0%/Exento en la tabla de totales | `invoice_processor/ride_builder.py:140-146` (`totals_rows`) solo imprime "IVA 15%" e "IVA 5%" — `document.iva_0` existe y esta poblado por `aggregate_line_totals`, pero nunca aparece en el PDF. Una factura/NC con lineas exentas o tasa 0% no muestra ese desglose al comprador aunque si esta calculado y persistido. |
...
| Frontend: `toNumber()`/`round2()` duplicados | `features/documents/form.ts:136-139,149-151` y `creditNoteForm.ts:108-111,113-115` copian las mismas dos funciones en vez de importarlas desde `utils.ts` (que ya existe en la misma feature). |
| Frontend: `EstablishmentCard.tsx` reinventa `Card`/`Badge` | Reimplementa a mano el wrapper de card (`card`/`addForm`/`pointRow`/`emptyPoints`) en vez de `components/ui/Card.tsx`, y dos "pills" (`testingPill`, `codePill`) que son exactamente un `Badge` con `variant="warning"`/`primary` — deberia usar los componentes centralizados. El patron "panel inline crear/editar" (useState local + `FormField` + Button + `ApiErrorBanner`) se repite 3 veces casi identico dentro del mismo archivo/`EstablishmentsScreen.tsx` sin abstraerse. La fila de punto de emision tampoco usa `EntityAvatar`/`ListCell` de `ListItemPrimitives.tsx` pese a calzar con ese patron. |

## Deuda Solventada

- 2026-06-28: `POST /documents/{id}/annul` ya bloquea facturas con
  `annulled_by_credit_note_id` antes de cambiar estado, emitir eventos o tocar
  integraciones. El bloqueo vive en `AnnulDocumentUseCase` como regla de negocio y en
  `DynamoDocumentsRepository.annul()` como `ConditionExpression` defensiva para cerrar
