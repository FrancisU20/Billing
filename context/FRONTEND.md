...
| `ListScreenHeader.tsx` | encabezado compartido de pantallas de listado: icono+kicker, titulo, boton primario opcional (`action`) y boton secundario opcional (`secondaryAction`, variant `outline` — ej. "Nota de crédito" junto a "Emitir documento" en Documentos). Cada pantalla sigue armando su propia fila de metricas (`StatMetric`) y filtros (`FilterBar`) debajo. Usado por las 5 `*ListScreen.tsx` (clients/products/tenants/plans/documents). |
