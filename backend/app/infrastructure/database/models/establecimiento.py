...
class PuntoEmisionModel(Base):
    ...
    establecimiento_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("establecimientos.id"), nullable=False
    )
    codigo: Mapped[str] = mapped_column(String(3), nullable=False)
    estado: Mapped[str] = mapped_column(String(20), default="ACTIVE")
...
class SecuencialModel(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    punto_emision_id: Mapped[uuid.UUID] = mapped_column(
        UUID(as_uuid=True), ForeignKey("puntos_emision.id"), nullable=False
    )
