...
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `context/BACKEND.md`       | Auditoria 2026-06-27: `BaseRepository` sin soporte transaccional; construccion de "lock item" para unicidad aun manual; filtro `q` reimplementado 4 veces sin clase base; `EventPublisher` (fire-and-forget) y `outbox.py` conviven sin guia de cuando usar cada uno; cache de secrets sin techo; prefix de secret duplicado entre codigo e infra CDK; 3 migraciones duplican el mismo loop de scan; piezas de `shared/` sin test unitario propio. Solventado 2026-06-28: `Tenant.subscription_status` como `SubscriptionStatus(StrEnum)`. Solventado 2026-06-29: `CancellationReasons`, deteccion de lock fallido y `Select=COUNT` paginado centralizados en `shared/db` |
