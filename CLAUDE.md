...
`/credit-notes`) — ya no es un boton dentro de Documentos. **Revisado otra vez (Sprint
2-3, 2026-06-26):** ese modulo ya no lista facturas elegibles, lista las NC emitidas
(cualquier estado) con boton "Crear nota de credito" que abre el picker de factura; se
agrego ademas un mecanismo real de **reintento** (`POST /documents/{id}/retry`,
`RetryDocumentUseCase`) para documentos `REJECTED` — reusa la misma clave de
acceso/secuencial en vez de obligar a emitir uno nuevo (la Ficha Tecnica del SRI lo exige
asi), visible en Documentos y en Notas de Credito (mismo `DocumentListItem.tsx`); y se
...
vivo de esa NC y oculta "Anular factura" cuando ya hay una anulacion en curso/hecha. El
banner anterior solo vivia en el detalle — reporte real del usuario ("no lo veo") llevo a
extenderlo a los listados (Documentos y Notas de Credito): badge "Anulada por NC" +
`GET /documents` ahora resuelve, en una sola tanda por pagina (`repo.get_many` +
`handler.py::_attach_linked_sequentials`), el secuencial legible del documento vinculado
(que factura acredita una NC, que NC anula una factura) sin N+1 fetches en el frontend,
incluso para documentos historicos sin backfill. `InvoicePickerModal` ya no deja
seleccionar facturas ya anuladas. Detalle completo en `context/INVOICES.md` secciones
"Nota De Credito (04)", "Reintento De Documentos Rechazados", "Marcado De Factura Anulada
Por NC" y "Visibilidad De Anulacion En Listados".
Hub "Mi empresa"
...
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `context/BACKEND.md`       | Auditoria 2026-06-27: piezas de `shared/` sin test unitario propio. Solventado 2026-06-28: `Tenant.subscription_status` como `SubscriptionStatus(StrEnum)`. Solventado 2026-06-29: `BaseRepository._transact_write_items()` para repos tenant-scoped; `CancellationReasons`, deteccion de lock fallido y `Select=COUNT` paginado centralizados en `shared/db`; helper comun para locks de unicidad; prefix de certificados solo viene de CDK/env; regla explicita outbox vs `EventPublisher(DirectPublishReason)`; scan paginado de migraciones centralizado en `migrations/scanner.py`; parseo HTTP de `limit`/enums centralizado en `lambdas/_base/query_params.py`; busqueda textual `q` centralizada en `shared/search.py`; cache LRU acotado para Secrets Manager |
| `context/FRONTEND.md`      | `SelectField` sin construir; tests de render bloqueados por infra; busquedas backend a escala pendientes. Auditoria 2026-06-27: colores 100% limpios (sin hex/rgb hardcodeado), pero `sizes`/iconos sin escala de tamano de control; `Screen.tsx` codigo muerto; `Card`/`Badge`/`EntityAvatar` reinventados en multiples features; mensajes de validacion Zod sin centralizar; `auth` no adopto `useFormSubmit`; polling y banners de auto-retry reimplementados por feature sin hook base; varias pantallas de flujo largo (300-580 lineas) violan SRP. Solventado 2026-06-29: `FormSection` centralizado en `components/ui`                                                                                                                                     |
| `context/AUTH.md`          | Sin tests de integracion Cognito real; evaluar rotacion de refresh token. Auditoria 2026-06-27: `try/finally` enganoso en `confirm_forgot_password`; `user_exists()` usa permisos de admin de mas; constantes de OTP no compartidas con `onboarding`; frontend con `{loading,error}` reimplementado a mano en los 4 hooks                                                                                                                  |
| `context/TENANTS.md`       | Listado admin con scan para alta cardinalidad/dashboard; Scan completo de `payments` para ingresos del dashboard superadmin sin techo. Auditoria 2026-06-27: `_dlocal_client` en el handler acopla `tenants` a `subscriptions`; 3 `Result` dataclasses estructuralmente iguales sin unificar; frontend con `TenantDashboardScreen.tsx` (584 lineas) reimplementando `formatCurrency`. Solventado 2026-06-28: `subscription_status` modelado como `StrEnum`. Solventado 2026-06-29: `TenantCard.tsx` eliminado                                                                  |
| `context/PLANS.md`         | `list()` con scan completo aceptable solo para catalogo chico. Auditoria 2026-06-27: `Plan` es la unica entidad global sin heredar `GlobalEntity` y sin soft delete; `list()` sin paginacion real (rompe el contrato `ApiResponse.paginated` de los demas dominios); regex de slug duplicado en 2 archivos; `PlanCard` reinventa `Card` en frontend. Solventado 2026-06-29: `PlanForm` usa `FormSection` compartido                                                                                                                                                                                                                                   |
| `context/CLIENTS.md`       | Busqueda `q` y validacion batch no escalan para cargas masivas. Auditoria 2026-06-27: `CancellationReasons` duplicado inline; `_derive_person_type` atado al dominio, no reusable desde `tenants`; `CLIENTS_PAGE_SIZE` muerta en frontend; pantallas de detalle/edicion sin cubrir el caso "id inexistente sin error"                                                                                                                  |
| `context/PRODUCTS.md`      | Inventario avanzado con movimientos/reservas queda para sprint futuro. Auditoria 2026-06-27: `invoice_code` no determinista si falta backfill; `discount_campaign_repository.save()` rompe la convencion `commit()` del resto; frontend con `ProductPickerModal` casi duplicado de `ClientPickerModal` de `documents`, `formatCurrency` centralizado nunca usado (5 sitios con `toFixed`), avatar sin `EntityAvatar`                                                                                                                                                       |
| `context/ONBOARDING.md`    | Queue dedicada Enterprise automatica es alcance futuro. Auditoria 2026-06-27: codigo muerto confirmado (`i_payment_verifier`, `payment_verifier.py`, 3 errores de dominio sin uso, vestigio pre-modelo-Netflix); frontend no usa el `expires_at` del OTP para countdown. Solventado 2026-06-28: `is_free` usa `Decimal` en plan catalog. Solventado 2026-06-29: `RegistrationForm` usa `FormSection` compartido                                                                                                                                          |
| `context/CERTIFICATES.md`  | Movil nativo, ampliacion de CAs y costo a escala son decisiones futuras. Solventado 2026-06-29: logs de Secrets Manager en `CertificateStore` incluyen detalle de excepcion sin exponer p12/password; prefix de Secrets Manager viene de `CERTIFICATE_SECRET_PREFIX` inyectado por CDK                                                                                                                                                                                                                  |
| `context/SUBSCRIPTIONS.md` | Scans en workers aceptables hasta ~10K. Auditoria 2026-06-27: banners de auto-retry duplicados y pantalla de activacion larga. Solventado 2026-06-28: endpoint publico de pago requiere `X-Checkout-Token` y no expone PII en polling; cobro automatico crea `AUTO_RENEWAL_RECONCILIATION`; webhook dLocal ingresa por SQS con DLQ/alarma y update condicional; `stale_payment_cleaner` cancela orders antiguos; `orphan_payment_notifier` marca `orphan_alerted_at`; `refund_payment` loguea detalle; confirmacion valida cedula/RUC; `use3dsFlow` cancela polling; dLocal cuantiza montos; `plan_net_price` centraliza precio neto; cambio de plan bloquea pagos en curso.                                                                                                                  |
| `context/INVOICES.md`      | Eventos de `invoice_processor` van por SQS directo, no por outbox (unico dominio critico fuera de ese mecanismo); literal SOAP de rechazo sin verificar contra SRI real; clasificacion de errores SRI parcial; colas dedicadas enterprise sin auto-provision; sin concurrencia reservada (cuenta AWS limitada a 10 ejecuciones concurrentes en `sa-east-1`); plazo de anulacion sin ajuste por feriados de Ecuador (deliberado); XSD de NC sin probar contra `celcer.sri.gob.ec`; reintento manual solo cubre `REJECTED`; NC de anulacion en `FAILED_PERMANENT` bloquea la factura para siempre (deliberado); frontend con listados de Documentos/NC casi duplicados y polling inconsistente entre listado y detalle. Solventado 2026-06-28: `POST /documents/{id}/annul` bloquea facturas con `annulled_by_credit_note_id` en use case y repositorio. Solventado 2026-06-29: RIDE incluye fila IVA 0%/Exento; calculos monetarios por linea y escalado de NC centralizados en `domain/totals.py`; previews frontend de documentos comparten `toNumber()`/`round2()` con redondeo alineado a backend. |
