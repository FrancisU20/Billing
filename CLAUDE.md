...
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `context/BACKEND.md`       | Auditoria 2026-06-27 (nueva seccion): `BaseRepository` sin soporte transaccional; extraccion de `CancellationReasons` duplicada en 5 repos en vez de usar `shared/db/transactions.py`; patron de "lock item" para unicidad reimplementado 4 veces sin mixin comun; filtro `q` reimplementado 4 veces sin clase base; `EventPublisher` (fire-and-forget) y `outbox.py` conviven sin guia de cuando usar cada uno; cache de secrets sin techo; prefix de secret duplicado entre codigo e infra CDK; 3 migraciones duplican el mismo loop de scan; piezas de `shared/` sin test unitario propio. Solventado 2026-06-28: `Tenant.subscription_status` como `SubscriptionStatus(StrEnum)` |
| `context/FRONTEND.md`      | `SelectField` sin construir; tests de render bloqueados por infra; busquedas backend a escala pendientes. Auditoria 2026-06-27: colores 100% limpios (sin hex/rgb hardcodeado), pero `sizes`/iconos sin escala de tamano de control; `Screen.tsx` codigo muerto; `Card`/`FormSection`/`Badge`/`EntityAvatar` reinventados en multiples features; mensajes de validacion Zod sin centralizar; `auth` no adopto `useFormSubmit`; polling y banners de auto-retry reimplementados por feature sin hook base; varias pantallas de flujo largo (300-580 lineas) violan SRP                                                                                                                                     |
...
| `context/ONBOARDING.md`    | Queue dedicada Enterprise automatica es alcance futuro. Auditoria 2026-06-27 (nueva seccion): codigo muerto confirmado (`i_payment_verifier`, `payment_verifier.py`, 3 errores de dominio sin uso, vestigio pre-modelo-Netflix); frontend reinventa `FormSection` y no usa el `expires_at` del OTP para countdown. Solventado 2026-06-28: `is_free` usa `Decimal` en plan catalog.                                                                                                                                          |
